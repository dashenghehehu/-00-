...
class RegisterPanel:
    ...
    def show(self):
        ...
        name_img = PhotoImage(file="image\\user.png", master=self.reg_frame)
        ...
        name_img_label = Label(form_frame, image=name_img, width=30, height=30, bg="#333333")
        ...
        name_img_label.grid(row=0, column=0, padx=5)
        user_img_label.grid(row=1, column=0, padx=5)
        key_img_label.grid(row=2,column=0, padx=5)
        confirm_img_label.grid(row=3, column=0, padx=5)
        Label(form_frame, text="昵称：", font=("宋体", 12), bg="#333333", fg="white") \
        ...
        self.name = StringVar()
        Entry(form_frame, textvariable=self.name, bg="#e3e3e3", width=30) \
    ...
    # 获取输入的用户名、密码、确认密码
    def get_input(self):
        return  self.name.get(), self.user.get(), self.key.get(), self.confirm.get()
